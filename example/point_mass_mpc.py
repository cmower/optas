...
class Controller:
    ...
    def next_state(self, curr, dcurr, goal, obs):
        ...
        if not self.solver.did_solve():
            for vc_collection in self.solver.violated_constraints(
                self.solution, params
            ):
                for vc in vc_collection:
                    print(vc)
            raise RuntimeError("solver failed")
        plan_y = self.solver.interpolate(
            self.solution[f"{self.pm_name}/y"], self.duration
        )
        plan_dy = self.solver.interpolate(
            self.solution[f"{self.pm_name}/dy"], self.duration
        )
...
class Animate:
    def __init__(self, animate):
        ...
        (self.mpc_line,) = self.ax["birdseye"].plot(
            [], [], "-x", color="yellow", label="mpc"
        )
        ...
        self.obs_visual = plt.Circle(
            self.obs_pos, radius=self.planner.obs_rad, color="black"
        )
    ...
    def update(self, frame):
        ...
        self.curr, self.dcurr, plan_y, plan_dy = self.controller.next_state(
            self.curr, self.dcurr, goal, obs
        )
