...
vis = optas.Visualizer()
vis.grid_floor()
# vis.robot(
#     robot, q=q_solution, show_links=True, display_link_names=True, link_names_alpha=0.4
# )  # nominal
vis.robot(
    robot, q=q_solution, show_links=True, display_link_names=True, link_names_alpha=0.4
)  # solution
...
vis.sphere(position=p_goal.toarray().flatten().tolist(), radius=0.05, alpha=0.5, rgb=[0, 1, 0])
vis.start()
