...
class DualKukaPlanner:
    ...
    def __init__(self):
        ...
        Ql = builder.get_model_states(kukal_name)  # ndof-by-T sym array for robot trajectory
        Qr = builder.get_model_states(kukar_name)  # ndof-by-T sym array for robot trajectory
        ...
        pos2l = pos1l + optas.DM([0., 0., 0.3])
        pos2r = pos1r + optas.DM([0., 0., 0.3])
        ...
        for i in range(T):
            ...
            if alpha_ < 0.4:
                ...
                path_eel[:, i] = alpha*pos1l + (1.-alpha)*pos0l
                path_eer[:, i] = alpha*pos1r + (1.-alpha)*pos0r
                ...
                path_eel[:, i] = alpha*pos2l + (1.-alpha)*pos1l
                path_eer[:, i] = alpha*pos2r + (1.-alpha)*pos1r
