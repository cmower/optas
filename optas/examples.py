...
class SimpleJointMotionPlanner:
    ...
    def __init__(self,
        ...
        if qlim:
            builder.add_bound_inequality_constraint('qlim', self.robot_model.lower_actuated_joint_limits, Q, self.robot_model.upper_actuated_joint_limits)
        if qdlim is not None:
            builder.add_bound_inequality_constraint('qdlim', -fabs(qdlim), dQ, fabs(qdlim))
        for link in self.robot_model.link_names:
            ...
            if isinstance(xlim, (list, tuple)):
                builder.add_bound_inequality_constraint(f'xlim_{link}', xlim[0], P[0,:], xlim[1])
            if isinstance(ylim, (list, tuple)):
                builder.add_bound_inequality_constraint(f'ylim_{link}', ylim[0], P[1,:], ylim[1])
            if isinstance(zlim, (list, tuple)):
                builder.add_bound_inequality_constraint(f'zlim_{link}', zlim[0], P[2,:], zlim[1])
    ...
    def plan(self, qa, qb):
        ...
        plan = self.solver.interpolate(solution[f'{self.name}/q'], 1., fill_value='extrapolate')
        return success, plan
